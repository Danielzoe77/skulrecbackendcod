...
const creatRecord = asyncHandler (async (req,res) =>{
  ...
  if (!user) {
    ...
    const record = await Record.create({
        className, studentName, amountToBePaid, amountDeposited, balanceAmount, userId,});


   if (record) {
      const { _id, className, studentName, amountToBePaid, amountDeposited, balanceAmount,userId} = record;
      res.status(201).json({ _id, className, studentName, amountToBePaid, amountDeposited, balanceAmount, userId});
   } else {
       res.status(400);
...
const updateRecord = asyncHandler(async(req,res) => {
    ...
    if(record){
        const{_id, className, studentName, amountToBePaid, amountDeposited, balanceAmount }= record
